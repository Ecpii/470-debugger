...
impl ICache {
    pub fn new(base: &str, snapshots: &Snapshots) -> Option<Self> {
        ...
        snapshots.get_var(&format!("{base}.dbg_this_is_icache"))?;
    ...
    fn get_outputs(&self, snapshots: &Snapshots) -> Line {
        ...
        let data = snapshots
            .get_var(&format!("{}.data.dbbl_level", self.base))
            .unwrap()
            .as_hex();
...
impl StatefulWidget for ICache {
    ...
    fn render(
        ...
        let title = Line::from("Instruction Cache").bold().centered();
